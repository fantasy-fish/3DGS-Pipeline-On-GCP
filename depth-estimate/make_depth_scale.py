...
def get_scales(key, cameras, images, points3d_ordered, depths_dir):
    ...
    invmonodepthmap = cv2.imread(f"{depths_dir}/{image_meta.name[:-n_remove]}.png", cv2.IMREAD_UNCHANGED)
...
def compute_depth_scales(base_dir, depths_dir, model_type="bin"):
    """计算深度图的尺度和偏移量"""
    cam_intrinsics, images_metas, points3d = read_model(os.path.join(base_dir, "sparse", "0"), ext=f".{model_type}")
    ...
    depth_param_list = Parallel(n_jobs=-1, backend="threading")(
        delayed(get_scales)(key, cam_intrinsics, images_metas, points3d_ordered, depths_dir) for key in images_metas
    )
    ...
    with open(f"{base_dir}/sparse/0/depth_params.json", "w") as f:
        json.dump(depth_params, f, indent=2)

    print("make_depth_scale.py.compute_depth_scales Depth scales computation completed.")

# 示例调用
if __name__ == '__main__':
    # 示例参数
    base_dir = "../data/big_gaussians/standalone_chunks/campus"
    depths_dir = "../data/big_gaussians/standalone_chunks/campus/depths_any"
    model_type = "bin"

    compute_depth_scales(base_dir, depths_dir, model_type)
