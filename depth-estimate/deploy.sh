...
docker build -t depth-estimate-image .
docker tag depth-estimate-image us-central1-docker.pkg.dev/threed-reconstruction-431616/gs-pipeline/depth-estimate-image:latest
gcloud artifacts repositories create gs-pipeline \
    --repository-format=docker \
    --location=us-central1 \
    --description="Repository for gs-pipeline"

docker push us-central1-docker.pkg.dev/threed-reconstruction-431616/gs-pipeline/depth-estimate-image:latest

# Update the cloud run service
gcloud beta run deploy  depth-estimate \
  --image us-central1-docker.pkg.dev/threed-reconstruction-431616/gs-pipeline/depth-estimate-image:latest \
  --region us-central1
...
gcloud beta run deploy  depth-estimate \
  --image us-central1-docker.pkg.dev/threed-reconstruction-431616/gs-pipeline/depth-estimate-image \
...
gcloud beta run services update  depth-estimate \
  --add-volume name=once3d-input-volume,type=cloud-storage,bucket=once3d-output \
