...
docker tag run-3dgs-image us-central1-docker.pkg.dev/threed-reconstruction-431616/gs-pipeline/run-3dgs-image:latest
gcloud artifacts repositories create gs-pipeline \
    --repository-format=docker \
    --location=us-central1 \
    --description="Repository for gs-pipeline"

docker push us-central1-docker.pkg.dev/threed-reconstruction-431616/gs-pipeline/run-3dgs-image:latest

# Update the cloud run service
gcloud beta run deploy run-3dgs \
  --image us-central1-docker.pkg.dev/threed-reconstruction-431616/gs-pipeline/run-3dgs-image:latest \
  --region us-central1
...
gcloud beta run deploy run-3dgs \
  --image us-central1-docker.pkg.dev/threed-reconstruction-431616/gs-pipeline/run-3dgs-image \
...
gcloud beta run services update run-3dgs \
...
gcloud run services add-iam-policy-binding run-3dgs \
  --member="allUsers" \
...
# Update a trigger
gcloud eventarc triggers update video-upload-trigger2 \
  --location=asia-northeast1 \
  --destination-run-service=run-meshroom \
  --destination-run-region=us-central1 \
  --event-filters="type=google.cloud.storage.object.v1.finalized" \
